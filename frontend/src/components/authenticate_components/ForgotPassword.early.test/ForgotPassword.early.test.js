...
describe('ForgotPassword() ForgotPassword method', () => {
    ...
    describe('Happy Path', () => {
        ...
        test('submits valid email and shows success message', async () => {
            ...
            await waitFor(() => {
                expect(axios.post).toHaveBeenCalledWith('http://localhost:5000/api/authenticate/forgotpassword', { email: 'test@example.com' });
    ...
    describe('Edge Cases', () => {
        ...
        test('shows error message for non-existent email', async () => {
            ...
            await waitFor(() => {
                expect(screen.getByText('Not Existed Email In System.')).toBeInTheDocument();
            });
        });
